...
exports.verifyCandidate = function(req,res){
    mysqlconnection.query(`UPDATE CANDIDATE SET verified = 0 WHERE userID = ${req.params.ID};`,function(err,row,fields){
        if(!err){
            res.send("updated");
        } else {
            res.send("err");
...
exports.getUnverified = function(req,res){
    mysqlconnection.query(`SELECT * FROM CANDIDATE WHERE CANDIDATE.verified = 1;`,function(err,rows,fields){
        res.send(rows);
    });
}