...
exports.getQuestionsAsked = function(req,res){
    mysqlConnection.query(`SELECT * FROM USER JOIN CANDIDATE_QUESTION ON USER.id = CANDIDATE_QUESTION.askerId WHERE USER.id = ${req.params.userID};`,function(err,rows,fields){
        if(err){
            res.send(404);
        } else {
            res.send(rows);
